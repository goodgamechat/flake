defmodule FlakeTest do
  ...
  test "counter restart doesnt allow duplicate flake ids" do
    Flake.start(0)
    results = for _i <- 1..100_000 do
      {:ok, flake} = Flake.get_id(0)
      flake
    end
    count = Enum.uniq(results)
      |> Enum.count
  ...
  test "can generate 1,000,000 ids within a couple of seconds" do
    ...
    {time, _} = :timer.tc(fn ->
      ts = for j <- 0..(workers-1), do: Task.async(fn -> for _i <- 0..work, do: Flake.get_id(j) end)
